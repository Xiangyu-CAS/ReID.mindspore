...
# ============================================================================
'''cifar_resnet50
The sample can be run on Ascend 910 AI processor and GPU.
'''
import os
import random
import argparse
...
def create_reid_dataset():
    ...
    from lib.dataset.data_loader import get_train_loader
    from lib.losses.build_loss import build_loss_fn
    config_file = '../configs/naic20.yml'
    cfg.merge_from_file(config_file)
    cfg.freeze()
    dataset = prepare_multiple_dataset(cfg, None)
    de_dataset = get_train_loader(dataset.train, cfg)
...
if __name__ == '__main__':
    ...
    from resnet import Encoder, Head
    # net = resnet50(args_opt.batch_size, args_opt.num_classes)
    net = Head(Encoder(), args_opt.num_classes)
    ls = SoftmaxCrossEntropyWithLogits(sparse=True, reduction="mean")
    #opt = Momentum(filter(lambda x: x.requires_grad, net.get_parameters()), 0.01, 0.9)
    opt = Adam(filter(lambda x: x.requires_grad, net.get_parameters()), 3.5e-4)
    model = Model(net, loss_fn=ls, optimizer=opt, metrics={'acc'})

    # as for train, users could use model.train
    if args_opt.do_train:
        #dataset = create_dataset()
        dataset = create_reid_dataset()
